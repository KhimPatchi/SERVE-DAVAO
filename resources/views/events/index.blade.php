            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
                ...
                <div class="grid grid-cols-7 gap-1 text-center">
                    ...
                    @for($day = 1; $day <= $lastDayOfMonth->day; $day++)
                        @php
                            $currentDate = $today->copy()->setDay($day)->startOfDay();
                            ...
                            $isPastDate = $currentDate->isPast() && !$isToday;
                            ...
                            // Compute classes dynamically to avoid nested ternaries in HTML
                            $dayClass = 'relative h-8 flex items-center justify-center text-sm ';
                            if ($isToday) {
                                $dayClass .= 'bg-blue-500 text-white rounded-full font-bold ring-2 ring-blue-200';
                            } elseif ($isPastDate) {
                                if ($isUserAttendedPast) {
                                    $dayClass .= 'bg-gray-100 text-gray-400/60 rounded-lg border border-gray-200/50 opacity-60';
                                } else {
                                    $dayClass .= 'text-gray-400 opacity-50';
                                }
                            } elseif ($isUserAttendingUpcoming) {
                                $dayClass .= 'bg-green-50 rounded-lg border border-green-200 text-gray-700';
                            } else {
                                $dayClass .= 'text-gray-700';
                            }
                        @endphp
                        <div class="{{ $dayClass }}">
                            <span class="{{ $isToday ? 'text-white' : ($isPastDate ? 'text-gray-400' : '') }}">{{ $day }}</span>

                            <!-- Event Indicators -->
                            @if($hasEvents || $hasUserEvents)
                                <div class="absolute -top-1 -right-1 flex gap-0.5 {{ $isPastDate ? 'opacity-40' : '' }}">
                                    @if($isUserHosting)
